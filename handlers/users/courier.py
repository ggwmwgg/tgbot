...
async def admin_courier(query: types.CallbackQuery, state: FSMContext):
    ...
    if data == 'refresh':
        ...
        await dp.bot.send_message(id, _("Выберите опцию"), reply_markup=main_menu)
        await state.finish()
        ...
        if order.p_type == "Cash":
            ...
            if order.status == 1 or order.status == 2 or order.status == 3:
                ...
                courier_ing.add(types.InlineKeyboardButton(_("Забрал ✔"), callback_data="delivery"),
                                types.InlineKeyboardButton(_("Локация 📍"), callback_data="location"),
                ...
                courier_ed.add(types.InlineKeyboardButton(_("Доставил ✔"), callback_data="delivered"),
                               types.InlineKeyboardButton(_("Локация 📍"), callback_data="location"),
                               types.InlineKeyboardButton(_("Назад 🔙"), callback_data='back'))
...
async def c_process(query: types.CallbackQuery, state: FSMContext):
    ...
    if data == 'back':
        ...
        user = await quick_commands.select_user(order.user_id)
        user_text = _("Ваш заказ №%s доставляется курьером.\n\n") % order.id
        ...
        if order.p_type == "Cash":
            await query.message.delete()
            text = _("<b>Спасибо за доставку!</b>\n\nБыл ли данный заказ (%s) оплачен?")
            text = text % order.id

            courier_yn = types.InlineKeyboardMarkup(row_width=2)
            courier_yn.row(types.InlineKeyboardButton(_("Да ✔"), callback_data="yes"),
                           types.InlineKeyboardButton(_("Нет ❌"), callback_data="no"))
            ...
            orders = await quick_commands.select_all_orders_courier(user.id)
            order = await quick_commands.select_order_by_id(order.id)
            user_text = _("Ваш заказ №%s был доставлен.\n\n<i>Спасибо за заказ!</i>") % order.id
            await dp.bot.send_message(order.user_id, user_text)
            orders_list = types.InlineKeyboardMarkup(row_width=1, one_time_keyboard=True)
            count = 0
            count_all = 0
            ...
            orders_list.add(types.InlineKeyboardButton(_("Обновить список"), callback_data="refresh"))
            orders_list.add(types.InlineKeyboardButton(_("Назад 🔙"), callback_data="back"))
            greeting = _("<b>Заказ №%s доставлен! Спасибо.</b>\n\nВам назначено <b>%s</b> заказов(а).\nВсего заказов: <b>%s</b>\n\n<i>Выберите заказ из списка ниже:</i>\n")
...
async def process_paid(query: types.CallbackQuery, state: FSMContext):
    ...
    if data == "yes":
        ...
        orders = await quick_commands.select_all_orders_courier(user.id)
        order = await quick_commands.select_order_by_id(order.id)
        user_text = _("Ваш заказ №%s был доставлен и оплачен.\n\n<i>Спасибо за заказ!</i>") % order.id
        await dp.bot.send_message(order.user_id, user_text)
        orders_list = types.InlineKeyboardMarkup(row_width=1, one_time_keyboard=True)
        count = 0
        count_all = 0
        ...
        orders_list.add(types.InlineKeyboardButton(_("Обновить список"), callback_data="refresh"))
        orders_list.add(types.InlineKeyboardButton(_("Назад 🔙"), callback_data="back"))
        greeting = _("<b>Заказ №%s доставлен и оплачен! Спасибо.</b>\n\nВам назначено <b>%s</b> заказов(а).\nВсего заказов: <b>%s</b>\n\n<i>Выберите заказ из списка ниже:</i>\n")
        greeting = greeting % (order.id, count, count_all)
        await dp.bot.send_message(id, greeting, reply_markup=orders_list)
        await Admin.c_main.set()
    else:
        ...
        orders = await quick_commands.select_all_orders_courier(user.id)
        order = await quick_commands.select_order_by_id(order.id)
        user_text = _("Ваш заказ №%s был доставлен но не был оплачен.\n\n<i>Пожалуйста, свяжитесь с нами.</i>") % order.id
        await dp.bot.send_message(order.user_id, user_text)
        orders_list = types.InlineKeyboardMarkup(row_width=1, one_time_keyboard=True)
        count = 0
        count_all = 0
        ...
        orders_list.add(types.InlineKeyboardButton(_("Обновить список"), callback_data="refresh"))
        orders_list.add(types.InlineKeyboardButton(_("Назад 🔙"), callback_data="back"))
        greeting = _("<b>Заказ №%s доставлен без получения оплаты! Спасибо.</b>\n\nВам назначено <b>%s</b> заказов(а).\nВсего заказов: <b>%s</b>\n\n<i>Выберите заказ из списка ниже:</i>\n")
        greeting = greeting % (order.id, count, count_all)
        await dp.bot.send_message(id, greeting, reply_markup=orders_list)
        await Admin.c_main.set()
